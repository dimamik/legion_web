defmodule LegionWeb.AgentTracker.TelemetryTest do
  use ExUnit.Case
  ...
  describe "get_usage/1" do
    ...
    test "appends usage from the agent's own llm_stop events and broadcasts the full list" do
      insert_agent("used")
      Phoenix.PubSub.subscribe(LegionWeb.PubSub, "legion_web:agent:#{inspect("used")}")
    ...
    test "records no usage for llm_stop events that carry none" do
      insert_agent("no_usage")
      Phoenix.PubSub.subscribe(LegionWeb.PubSub, "legion_web:agent:#{inspect("no_usage")}")
    ...
    test "does not attribute a forwarded sub-agent llm_stop to the parent" do
      ...
      Phoenix.PubSub.subscribe(LegionWeb.PubSub, "legion_web:agent:#{inspect("child")}")
    ...
    test "records no usage while Legion usage tracking is disabled" do
      Application.put_env(:legion, :track_usage, false)
      on_exit(fn -> Application.delete_env(:legion, :track_usage) end)
      insert_agent("untracked")
      Phoenix.PubSub.subscribe(LegionWeb.PubSub, "legion_web:agent:#{inspect("untracked")}")
      usage = %{"input_tokens" => 1, "at" => 1}

      send(
        Telemetry,
        {:event, "untracked", :llm_stop, %{agent_id: "untracked", object: %{}, usage: usage}}
      )

      refute_receive {:usage, "untracked", _}, 200
      assert Telemetry.get_usage("untracked") == []
    end

    test "is not capped by the per-agent event limit" do
      insert_agent("busy")
      Phoenix.PubSub.subscribe(LegionWeb.PubSub, "legion_web:agent:#{inspect("busy")}")
